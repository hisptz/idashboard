...
export class VisualizationService {
  ...
  private _getSanitizedTableObject(table, tableConfiguration) {
    let newTable = _.clone(table);

    /**
     * Update table object with column total
     */
    if (tableConfiguration.showColumnTotal) {
      newTable = _.assign({}, this.addColumnTotal(newTable));
    }

    /**
     * Update table object with column sub total
     */
    if (tableConfiguration.showColumnSubtotal) {
      newTable = _.assign({}, this.addColumnSubTotal(newTable));
    }

    /**
     * Update table object with row total
     */
    if (tableConfiguration.showRowTotal) {
      newTable = _.assign({}, this.addRowTotal(newTable));
    }

    /**
     * Update table object with row sub total
     */
    if (tableConfiguration.showRowSubtotal) {
      newTable = _.assign({}, this.addRowSubtotal(newTable));
    }
