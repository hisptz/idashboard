...
export class DashboardEffects {
  ...
  portalConfigurationLoaded$ = this.actions$
    .ofType<portalConfiguration.LoadPortalConfigurationSuccessAction>(
  ...
  dashboardsLoaded$ = this.actions$
    ...
    .pipe(
      tap((action: any) => {
        ...
        const currentVisualization = action.payload.url.split("/")[5];
          ...
          const screenViewDashboardItems = _.flatten(
            _.take(
              _.map(action.payload.dashboards || [], (dashboardLoaded: any) =>
                _.take(
                  _.map(
                    dashboardLoaded.dashboardItems,
                    (dashboardItem: any) => {
                      return {
                        ...dashboardItem,
                        dashboardId: dashboardLoaded.id
                      };
                    }
                  ),
                  125
  ...
  createDashboard$ = this.actions$.pipe(
    ...
    switchMap((action: any) =>
      this._create(action.payload).pipe(
        map(
          (dashboardObject: any) =>
  ...
  renameDashboard$ = this.actions$.pipe(
    ...
    switchMap((action: any) =>
      this._rename(action.payload).pipe(
        map(
          (dashboardObject: any) =>
  ...
  @Effect({ dispatch: false })
  dashboardCreated$ = this.actions$.pipe(
  ...
  deleteDashboard$ = this.actions$.pipe(
    ...
    switchMap((action: any) =>
      this._delete(action.payload).pipe(
        map(
          (dashboardId: string) =>
            new dashboard.DeleteSuccessAction(dashboardId)
        )
      )
    )
  );
  ...
  @Effect({ dispatch: false })
  dashboardDeleted$ = this.actions$.pipe(
    ofType<dashboard.DeleteSuccessAction>(
      dashboard.DashboardActions.DELETE_SUCCESS
    ),
    withLatestFrom(this.store),
    tap(([action, store]: [dashboard.DeleteSuccessAction, AppState]) => {
  ...
  route$ = this.actions$.pipe(
    ...
    tap(([action, state]: [any, AppState]) => {
      const currentDashboardId = state.route.state.url.split("/")[3];
      if (currentDashboardId) {
        /**
  ...
  @Effect({ dispatch: false })
  dashboardItemAddedAction$ = this.actions$.pipe(
  ...
  @Effect({ dispatch: false })
  loadDashboardSharing = this.actions$.pipe(
  ...
  @Effect()
  bookmarkDashboard$ = this.actions$.pipe(
  ...
  loadDashboardNotification$ = this.actions$.pipe(
    ...
    switchMap(() =>
      this.httpClient.get("me/dashboards.json").pipe(
        map(
          (response: any) =>
            new dashboard.LoadDashboardNotificationSuccessAction(response)
        ),
        catchError(error =>
          of(new dashboard.LoadDashboardNotificationFailAction(error))
        )
      )
    )
  );
  ...
  private _getCurrentDashboardId(
    ...
    let currentDashboard = routeUrl.split("/")[3];
  ...
  private _loadSharingInfo(dashboardId: string): Observable<DashboardSharing> {
    return this.httpClient.get("sharing?type=dashboard&id=" + dashboardId).pipe(
