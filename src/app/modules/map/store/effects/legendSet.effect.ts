import { combineLatest as observableCombineLatest, of, Observable, forkJoin } from 'rxjs';
...
export class LegendSetEffects {
  ...
  loadLegendSets$ = this.actions$.ofType(legendSetAction.LOAD_LEGEND_SET).pipe(
    flatMap((action: visualizationObjectActions.CreateVisualizationObjectSuccess) => {
  ...
  loadAllLegendSets$ = this.actions$.ofType(legendSetAction.LOAD_LEGEND_SET_ALL).pipe(
    flatMap((action: legendSetAction.LoadAllLegendSet) => {
  ...
  @Effect()
  updateLegendSets$ = this.actions$.ofType(legendSetAction.UPDATE_LEGEND_SET).pipe(
  ...
  @Effect()
  changeOpacity$ = this.actions$.ofType(legendSetAction.CHANGE_LEGEND_SET_LAYER_OPACITY).pipe(
    map(
