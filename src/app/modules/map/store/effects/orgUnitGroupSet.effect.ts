...
export class OrganizationUnitGroupSetEffects {
  ...
  @Effect()
  addOrgUnitGroupSet$ = this.actions$.ofType(visualizationObjectActions.ADD_ORGANIZATIONUNITGROUPSET).pipe(
    flatMap((action: visualizationObjectActions.AddOrgUnitGroupSetVizObj) => {
      const layerIds = [];
      const orgGroupIds = [];
